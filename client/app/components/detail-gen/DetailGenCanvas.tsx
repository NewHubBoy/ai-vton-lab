...
export function DetailGenCanvas() {
  ...
  const handleDownload = useCallback(() => {
    ...
    link.download = 'ai-detail-page.png';
    link.click();
  }, [resultImage]);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center p-8 bg-gradient-to-br from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-800 rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700">
        <motion.div
          ...
          className="p-4 rounded-2xl bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/30 dark:to-orange-900/30 mb-4"
        >
          <FileImage className="w-8 h-8 text-amber-600 dark:text-amber-400" />
        </motion.div>
        <h3 className="text-base font-semibold text-zinc-800 dark:text-zinc-200">
          等待生成详情页
        </h3>
        <p className="text-sm text-zinc-500 mt-2 max-w-xs leading-relaxed">
          在左侧上传商品图片，选择风格和布局，然后点击「生成详情页」按钮
        </p>
      </div>
    );
  }
          ...
          >
            <div className="relative">
              <motion.div
                ...
                className="w-16 h-16 rounded-full border-4 border-amber-200 dark:border-amber-800 border-t-amber-600 dark:border-t-amber-400"
              />
              <motion.div
              ...
              >
                <FileImage className="w-6 h-6 text-amber-600 dark:text-amber-400" />
              </motion.div>
            </div>
